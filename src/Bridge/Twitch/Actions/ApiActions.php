 ...
 * ```
 *
 * Gated to {@see Access::Operator} — not to the broadcaster, and not to moderators.
 ...
 */
final class ApiActions implements ProvidesActions
{
    use UsesTwitch;
    ...
    public function actions(): array
    {
        return [
            new Action(
                ...
                // Ungrouped: there is only one of it, and a group holding a
                // single sub-command of the same name is a wasted level.
    ...
    {
        $dispatcher = $this->twitch($context)->getDispatcher();
    ...
    private function help(Context $context, Arguments $arguments): string
    {
        ...
        $signature = $this->twitch($context)->getDispatcher()->signature($call);
    ...
    private function call(Context $context, Arguments $arguments): PromiseInterface
    {
        ...
        return $this->twitch($context)->getDispatcher()->call($call, $this->namedFor($arguments))->then(
    ...
    private function render(Context $context, string $call, mixed $result): string
    {
        ...
        // A chat that cannot render a code block, with a few hundred characters
        // to play with, gets a compact one-liner instead of pretty-printed JSON
