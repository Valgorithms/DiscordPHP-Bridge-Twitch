 ...
 */
final class ChannelActions implements ActionProvider
{
    ...
    private function game(Context $context, Arguments $arguments): PromiseInterface
    {
        ...
        // nothing.
        return $context->bot->getTwitch()->games->fetchByName($wanted)->then(
    ...
    private function current(Context $context): PromiseInterface
    {
        ...
        return $context->bot->getTwitch()->channels->getMany([$id])->then(
            static function ($channels) use ($context): object {
                ...
                if ($channel === null) {
                    throw new ActionError(sprintf(
                        ...
                        $context->broadcasterLogin ?? 'that channel',
                    ));
                }
    ...
    {
        return $context->bot->getTwitch()->channels
            ->modify($context->requireBroadcaster(), $fields)
    ...
    {
        if (! $context->access->satisfies(Access::Broadcaster)) {
    ...
    {
        $login = $context->broadcasterLogin ?? 'that channel';
